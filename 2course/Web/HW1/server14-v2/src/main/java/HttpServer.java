...
//            response.setHeader("ETag", Long.toString(new Long(file.lastModified()).hashCode()));
            response.setHeader("ETag", Long.toString(Arrays.hashCode(body)));
            if (request.headers.containsKey("If-None-Match") &&
            request.headers.get("If-None-Match").equals(response.getHeaders().get("ETag"))) {
    ...
    private static final class Request {
        ...
        private Request(String request) throws UnsupportedEncodingException {
            ...
            firstLineTokens[1] = URLDecoder.decode(firstLineTokens[1], "UTF-8");

            if (firstLineTokens[1].endsWith("/")) {
                firstLineTokens[1] = firstLineTokens[1].substring(0, firstLineTokens[1].length() - 1);
            }
